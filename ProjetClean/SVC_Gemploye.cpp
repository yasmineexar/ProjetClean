...
namespace Composant
{
	...
	void CL_svc_gestionClients::CL_svc_gestionClients::modifier(int id_personne, String^ nom, String^ prenom, array<String^>^
		lesAdresses)
	{
		int i;
		...
		for (i = 0; i < lesAdresses->Length - 1; i++)
		{
			...
			this->adresse->setAdresse(lesAdresses[i]); i++;
			this->adresse->setVille(lesAdresses[i]); i++;
			this->adresse->setCp(lesAdresses[i]);this->cad->actionRows(this->adresse->UPDATE());
		}
	}
